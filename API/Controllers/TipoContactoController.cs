    ...
    public class TipoContactoController : ApiBaseController
{
    ...
    public TipoContactoController(IUserService userService, IUnitOfWork unitofwork, IMapper mapper)
    {
    ...
    // [ProducesResponseType(StatusCodes.Status200OK)]
    // [ProducesResponseType(StatusCodes.Status400BadRequest)]
    // public async Task<ActionResult<Pager<TipoContactoDto>>> GetPaginacion([FromQuery] Params usuarioParams)
    // {
    //     var entidad = await unitofwork.TipoContactos.GetAllAsync(usuarioParams.PageIndex, usuarioParams.PageSize, usuarioParams.Search);
    //     var listEntidad = mapper.Map<List<TipoContactoDto>>(entidad.registros);
    //     return new Pager<TipoContactoDto>(listEntidad, entidad.totalRegistros, usuarioParams.PageIndex, usuarioParams.PageSize, usuarioParams.Search);
    // }
