    ...
    public class ContratoController : ApiBaseController
{
    ...
    public ContratoController(IUserService userService, IUnitOfWork unitofwork, IMapper mapper)
    {
    ...
    // [ProducesResponseType(StatusCodes.Status200OK)]
    // [ProducesResponseType(StatusCodes.Status400BadRequest)]
    // public async Task<ActionResult<Pager<ContratoDto>>> GetPaginacion([FromQuery] Params usuarioParams)
    // {
    //     var entidad = await unitofwork.Contratos.GetAllAsync(usuarioParams.PageIndex, usuarioParams.PageSize, usuarioParams.Search);
    //     var listEntidad = mapper.Map<List<ContratoDto>>(entidad.registros);
    //     return new Pager<ContratoDto>(listEntidad, entidad.totalRegistros, usuarioParams.PageIndex, usuarioParams.PageSize, usuarioParams.Search);
    // }
